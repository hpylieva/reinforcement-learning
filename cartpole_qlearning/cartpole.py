...
class CartPoleQLearningAgent:
    def __init__(self,
        ...
        self._feature_bins = [
            ...
            # angle
            np.linspace(-0.5, 0.5, self._bin_sizes[2] + 1)[1:-1],
            # tip velocity
    ...
    def _build_state(self, observation):
        ...
        state = sum([np.digitize(feature_value, self._feature_bins[i]) * (self._max_bin_size + 1) ** i
                     for i, feature_value in enumerate(observation)])
        return state
        # ====================================================
